...
function getFactorial(n) {
  let fact = 1;
  if (n < 0 || n > 12) {
    throw new RangeError('The argument must be between 0 and .');
  }
  if (n === 0) {
    return fact;
  }
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if ((str.substring(i + 1).search(str[i]) === -1) && !seenChar.has(str[i])) {
      return str[i];
    }
    seenChar.add(str[i]);
  }
...
function isCreditCardNumber(ccn) {
  function sum(a, b) {
    let bb = b;
    if (bb >= 10) { bb = Math.floor(bb / 10) + (bb % 10); }
    return a + bb;
  }

  const check = parseInt(ccn.toString()[ccn.toString().length - 1], 10);
  const arr = [];
  let val = Math.floor(ccn / 10);
  while (val >= 10) {
    arr.unshift(val % 10);
  ...
  arr.reverse();
  return check === ((10 - (arr.reduce(sum, 0) % 10)) % 10);
}
...
function getDigitalRoot(num) {
  function getSum(total, n) {
    return total + parseInt(n, 10);
  }

  let tmp = num;
  while (tmp > 9) {
    tmp = tmp.toString().split('').reduce(getSum, 0);
  }
  return tmp;
}
...
function isBracketsBalanced(str) {
  ...
  while (arr.length > 0) {
    ...
    if (!brackets.has(check)) { return false; }
    if (!(check in bracketPairs)) {
      stack.push(check);
    } else if (bracketPairs[check] !== stack.pop()) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    const tmp = pathes[i].split('/');
    for (let j = 0; (j < arr.length) && (j < tmp.length); j += 1) {
      if (arr[j] !== tmp[j]) {
        arr.splice(j);
        arr.push('');
      }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    if (position[i][0] && position[i][0] === position[i][1] && position[i][2] === position[i][0]) {
      return position[i][0];
    }
    if (position[0][i] && position[0][i] === position[1][i] && position[2][i] === position[0][i]) {
      return position[0][i];
    }
  }
  if (position[0][0] && position[0][0] === position[1][1] && position[2][2] === position[0][0]) {
    return position[1][1];
  }
